package com.nj.mcp.demo.mcpplayground.sec09.host.config;
...
public class ApplicationConfiguration {
    ...
    public ChatClient chatClient(ChatClient.Builder builder,
                                 @Value("classpath:${section}/system-message.txt")
                                 Resource systemMessage,
                                 @Value("classpath:${section}/user-message.txt") Resource userMessage) {
    ...
    public Map<String, McpSyncClient> mcpClients(List<McpSyncClient> syncClients) {
        log.info("Sync client size: {}", syncClients.size());
