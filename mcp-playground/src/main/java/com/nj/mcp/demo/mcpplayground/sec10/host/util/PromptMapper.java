package com.nj.mcp.demo.mcpplayground.sec10.host.util;
...
public class PromptMapper {
    public static Prompt toPrompt(List<McpSchema.PromptMessage> promptMessages) {
        var messages = promptMessages.stream().map(PromptMapper::toMessage).toList();
        return Prompt.builder().messages(messages).build();
    }

    private static Message toMessage(McpSchema.PromptMessage promptMessage) {
        var content = extractContent(promptMessage.content());
    ...
    private static String extractContent(McpSchema.Content content) {
        return switch(content) {
            case McpSchema.TextContent  t -> t.text();
            case null, default -> throw new IllegalArgumentException("Expected text content");
        };
    }
}
